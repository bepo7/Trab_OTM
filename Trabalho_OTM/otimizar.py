...
# --- IMPORTS ---
import modelo_problema_setor  # Agora só importamos UM modelo
import config
...
def rodar_otimização(inputs, risco_maximo_usuario, lambda_aversao_risco, setores_proibidos=None):
    ...
    # Obtém o mapa de setores (necessário agora para o modelo unificado)
    mapa_setores = config.obter_mapa_setores_ativos()

    print("\n[GA] Inicializando Modelo Unificado...")
    
    # Instancia o modelo único. 
    # Se 'setores_proibidos' for None ou vazio, ele ignora a lógica de bloqueio internamente.
    ...
    res = minimize(
        ...
        seed=1,
        verbose=False # Mudei para False para limpar o terminal, mude para True se quiser ver o log
    )
    ...
    if res and res.X is not None:
        ...
        # df_final.to_csv(nome_arq, index=False, sep=';', decimal=',') # Opcional salvar CSV
